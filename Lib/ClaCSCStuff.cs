using System;

namespace MyPLC
{
    public class CrcStuff
    {
        static readonly byte[] CrcHi = {
        
        };

        static readonly byte[] CrcLo = {
        
        };

        public byte[] CalculateCrc(byte[] messageArray, int dataLength)
        {
            byte usCrcHi = 0xFF;
            byte usCrcLo = 0xFF;
            byte[] returnResult = { 0x00, 0x00, 0x00 };
            var messageIndex = 0;
            while (dataLength > 0)
            {
                var index = usCrcLo ^ messageArray[messageIndex];
                usCrcLo = Convert.ToByte(usCrcHi ^ CrcHi[index]);
                usCrcHi = CrcLo[index];
                messageIndex++;
                dataLength--;
            }
            //0th item is crcLo
            returnResult[0] = usCrcLo;
            //1st item is crcHi
            returnResult[1] = usCrcHi;
            //2nd item is the total CRC16.
            //returnResult[2] = Convert.ToByte((usCRCHi << 8 | usCRCLo));
            return returnResult;
        }

        public bool CheckCrc(byte[] messageToCheck, int numberOfBytes)
        {
            byte[] calculatedCrc = CalculateCrc(messageToCheck, numberOfBytes - 2);
            if (calculatedCrc[0] == messageToCheck[numberOfBytes - 2] && calculatedCrc[1] == messageToCheck[numberOfBytes - 1]) return true;
            return false;
        }
    }
}
